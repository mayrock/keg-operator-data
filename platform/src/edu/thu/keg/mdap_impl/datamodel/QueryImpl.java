...
public class QueryImpl implements Query {
	...
	public Query select(DataField ... fields) {
		if (this.getGroupByFields() != null)
			return new QueryImpl(transformFields(fields), this.wheres, this.orders, this.provider, this);
		else
			return new QueryImpl(fields, this.wheres, this.orders, this.provider, null);
	}
	@Override
	public Query where(String fieldName, Operator op, Object value) {
		...
		if (field instanceof AggregatedDataField) {
			...
			return new QueryImpl(transformFields(fields), wheres, this.orders, this.provider, this);
		} else {
			wheres.addAll(this.wheres);
			wheres.add(new WhereClause(field, op, value));
			return new QueryImpl(fields, wheres, this.orders, this.provider, null);
		}
	}
	@Override
	...
	public Query orderBy(String fieldName, Order order) {
		...
		return new QueryImpl(this.fields, this.wheres, orders, this.provider, null);
	}
	@Override
