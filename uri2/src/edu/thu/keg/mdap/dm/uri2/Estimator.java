...
public class Estimator {
	...
	private void computeP_URI(int m, int w, int l) {
		...
		for (int k = 0; k < trnModel.K; k++){
			trnModel.p[k] = (trnModel.nw[w][k] + trnModel.beta)/(trnModel.nwsum[k] + Vbeta) *
					(trnModel.ndl[m][l][k] + trnModel.alpha)/(trnModel.ndlsum[m][l] + Kalpha);
		}
	...
	public static void computeTheta_L(Model trnModel){
		double Kalpha = trnModel.K * trnModel.alpha;
		for (int m = 0; m < trnModel.M; m++){
			for (int l = 0; l < trnModel.L; l++) {
				for (int k = 0; k < trnModel.K; k++){
					trnModel.theta_l[m][l][k] = (trnModel.ndl[m][l][k] + trnModel.alpha) / (trnModel.ndlsum[m][l] + Kalpha);
				}
			}
		}
	}
	...
	public static void computeLocationDist(Model trnModel){
		double total = 0;
		for (int l = 0; l < trnModel.L; l++){
			total += trnModel.nlsum[l];
		}
		for (int l = 0; l < trnModel.L; l++){
			trnModel.location_dist[l] = trnModel.nlsum[l] / total;
	...
	public static void computePsi(Model trnModel){
		for (int l = 0; l < trnModel.L; l++){
			for (int k = 0; k < trnModel.K; k++){
				trnModel.psi[l][k] = (trnModel.nl[l][k] + trnModel.beta) / (trnModel.nlsum[l] + trnModel.K * trnModel.alpha);
			}
		}
	}
	
	public static void computeRPsi(Model trnModel) {
		//psi and location_dist should already be calculated
		for (int k = 0; k < trnModel.K; k++){
			double sum = 0;
			for (int l = 0; l < trnModel.L; l++){
				double t = trnModel.psi[l][k] * trnModel.location_dist[l];
				trnModel.rpsi[k][l] = t;
				sum += t;
			}
			for (int l = 0; l < trnModel.L; l++){
				trnModel.rpsi[k][l] /= sum;
			}
		}
	}
	...
	public static void computeAll(Model model) {
		...
		computePhi(model);
		computeTheta_L(model);
		computePsi(model);
		computeLocationDist(model);
		computeRPsi(model);
