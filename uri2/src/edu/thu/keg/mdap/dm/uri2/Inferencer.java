...
public class Inferencer {	
	...
	public Model inference(String [] strs){
		...
		LDADataset dataset = LDADataset.readDataSet(strs, option.V, option.L);
	...
	protected int infSampling(int m, int n){
		...
		int w = newModel.data.docs[m].words[n][0];
		int l = newModel.data.docs[m].words[n][1];
		...
		for (int k = 0; k < newModel.K; k++){			
			newModel.p[k] = (trnModel.nw[w][k] + newModel.nw[w][k] + newModel.beta)/(trnModel.nwsum[k] +  newModel.nwsum[k] + Vbeta) *
	...
	protected void computeNewPhi(){
		for (int k = 0; k < newModel.K; k++){
			for (int _w = 0; _w < newModel.V; _w++){
				newModel.phi[k][_w] = (trnModel.nw[_w][k] + newModel.nw[_w][k] + newModel.beta) / (newModel.nwsum[k] + newModel.nwsum[k] + trnModel.V * newModel.beta);
			}//end foreach word
		}// end foreach topic
	}
